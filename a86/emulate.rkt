...
(define (asm-emulate instructions #:entry-point [entry-point 'entry])
...
;; of the .text section.
(define (compute-label-addresses labels->indices hi-text-address)
  (for/hash ([(label index) (in-hash labels->indices)])
    (values label (word-aligned-offset hi-text-address (* -1 index)))))
    ...
    (match instructions
      [(list)
       ...
       (values text-contents data-contents rodata-contents bss-contents
               globals externs labels->indices)]
      [(cons instruction instructions)
       (let ([separate (λ (#:add-index [add-index   #t]  ;; This one is true!
         ...
         (match instruction
           ...
           [(Label l)  (separate #:add-index #f #:labels  (hash-set labels->indices l index))]
           [(or (Dd x) (Dq x))
            (match section
